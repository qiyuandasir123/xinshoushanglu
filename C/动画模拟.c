/* 辉夜姬 Live 字符动画（16:9 / 120x34）
 * 编译: gcc 动画模拟.c -o 动画模拟.exe -O2 -lwinmm
 * 运行: 双击 exe，或在 VS Code 里按运行键 —— 都能跑
 *
 * 会自动在 3 个位置里找 txt 文件夹，所以 VS Code 用什么工作目录都不怕:
 *     1) 和 exe 同一个文件夹      （D:\code\C\txt）
...
/* 探测可用的 txt 文件夹，把路径前缀写进 out；找到返回 1，没找到返回 0 */
static int find_txt(char *out, size_t n)
{
    ...
    for (int k = 0; k < 3; ++k) {
        ...
        FILE *f = fopen(cand, "r");
        if (f) { fclose(f); snprintf(out, n, "%s", base); return 1; }
    }
    snprintf(out, n, "txt");
    return 0;
}
...
int main(void) {
    ...
    if (!find_txt(dir, sizeof dir)) {
        printf("找不到 txt 文件夹（里面要有 1.txt、2.txt ...）\n");
        printf("请在下面任一位置放好 txt 文件夹后重试：\n");
        printf("  1) 和 exe 同一个文件夹\n  2) 当前工作目录\n  3) 当前工作目录的上一级\n");
        system("pause");
        return 1;
    }
